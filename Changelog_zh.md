...
----------------------------
新增：
- 新增 **Execution（执行）** 组件，用于在执行不确定性下处理来自规划的方案与执行方案。
- 支持多步 / 分阶段规划器执行，以及周期性规划器通信。
- 通过输入 JSON 中的 `delayConfig` 支持运行时延迟生成。
- 当 `outputScreen <= 2` 时，在输出 JSON 中新增延迟区间输出（`delayIntervals`）。
- 新增 `--prettyPrintJson` 选项，用于输出人类可读格式的 JSON。
- 通过输入 JSON 中的 `maxCounter` 与输出 JSON 中的 `agentMaxCounter` 支持可配置的动作计数器。
- 通过 `outputSegmentSize` 在输出 JSON 中提供压缩路径段元数据。

变更：
- 更新仿真器，采用由执行器驱动的规划/执行流程，而非直接消费返回的动作。
- 更新动作模型，支持基于智能体足迹（`agentSize`）的新碰撞检测模型，而非仅点占用。
- 更新输出 JSON，包含延迟信息、分段路径元数据以及当前 starter-kit 版本字符串。
- 更新时间控制，区分预处理时间、初始规划时间、规划器通信间隔、每动作执行时间与执行器处理方案的时间。
- 更新输入格式文档，说明 `delayConfig`、`maxCounter`、`--prettyPrintJson` 以及延迟区间输出。
