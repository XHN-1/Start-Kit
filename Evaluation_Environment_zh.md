...
当您请求评测时，我们将：
1. 从您的私有仓库克隆指定分支，
2. 用 start-kit 中的不可修改文件集覆盖（您本也不应修改它们），
3. 构建 Docker 环境并在其中编译您的代码，
4. 运行评测流程以计算排行榜分数。
...
提交将在按以下方式配置的 Docker 环境中进行评测：
- 基础镜像：[pytorch/pytorch:2.4.1-cuda11.8-cudnn9-devel](https://hub.docker.com/layers/pytorch/pytorch/2.4.1-cuda11.8-cudnn9-devel/images/sha256-ebefd256e8247f1cea8f8cadd77f1944f6c3e65585c4e39a8d4135d29de4a0cb?context=explore)
...
若使用 start-kit 中默认的 `apt.txt` 与 `pip.txt`，评测环境中将提供下列包。 
若您在 `apt.txt` 与 `pip.txt` 中添加了额外包，这些包也会被安装。

默认我们提供 Python 3.11.9 与 Pybind11 2.13.6。`pip.txt` 中列出的所有包将安装到该默认 Python3.11 环境。 
但请注意，Ubuntu 系统还提供 python3.10。不会为该环境安装包，建议不要使用。
